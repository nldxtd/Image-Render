...
Material *SceneParser::parseMaterial() {
    ...
    auto *answer = new Material(diffuseColor, specularColor, shininess, emission, refr, type);
    return answer;
}
...
Group *SceneParser::parseGroup() {
    ...
    answer->tree = new ObjectKDTree(&answer->vec_of_obj);
    getToken(token);
    assert (!strcmp(token, "}"));
